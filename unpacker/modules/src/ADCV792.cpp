...
bool ADCV792::looksLikeHeader(unsigned short* point, unsigned short* end) const{
	if (point + 1 >= end){
		return false;
	}
...
int ADCV792::peekSlot(unsigned short* point, unsigned short* end) const{
	if (point + 1 >= end){
		return -1;
	}
	return (point[1] >> 11);
...
unsigned short* ADCV792::parseEvent(unsigned short* point, unsigned short*){
	// Each 32-bit V792 hardware word is stored as two uint16: LO first, HI second.
	// Header:
...
bool ADCV792::readPhysicsEvent(unsigned short*& point, unsigned short* end){
	resetEventState();
...
bool ADCV792::decode(unsigned short*& point, unsigned short* end, EventData& event, const std::string& moduleName){
