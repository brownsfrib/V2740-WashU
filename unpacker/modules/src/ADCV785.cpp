...
bool ADCV785::looksLikeHeader(unsigned short* point, unsigned short* end) const{
	if (point + 1 >= end){
		return false;
	}
...
int ADCV785::peekSlot(unsigned short* point, unsigned short* end) const{
	if (point + 1 >= end){
		return -1;
	}
	return (point[1] >> 11);
...
unsigned short* ADCV785::parseEvent(unsigned short* point, unsigned short*){
	// Each 32-bit V785 hardware word is stored as two uint16: LO first, HI second.
	// Header:
...
bool ADCV785::readPhysicsEvent(unsigned short*& point, unsigned short* end){
	resetEventState();
...
bool ADCV785::decode(unsigned short*& point, unsigned short* end, EventData& event, const std::string& moduleName){
