...
// bit depth of ADC
// input voltages must range 0-2V (default)
// this gives us a ~30.5 uV binning on ADC values
#define bitDepth 65536.0
#define conversion 2e3/bitDepth;
...
void parseItem(CRingItem* item, TGraph** graphs){
    ...
    //const float bitDepth = 65536.0;
		graphs[channelNumber]->Set(0);
		for (uint32_t i = 0; i < nSamples; i++){
			uint16_t sample = (uint16_t)(frag[23 + i] & 0xFFFF);
      float mV = static_cast<float>(sample)*conversion;
			graphs[channelNumber]->SetPoint(i, i, mV);
		}
...
int main(int argc, char** argv){
	...
	for (int i=0; i < channelCount; i++){
		...
		can.cd(i+1);
		gPad->SetGrid();
		frames[i] = gPad->DrawFrame(0.1, 0.1, 128, 500, 
				Form("Channel %d;Time Buckets; mV", i));
